...
template<typename T>
inline int** insert (T* originalArray, int positionToInsertAt, T (&ValueToInsert), int sizeOfOriginalArrayX)
{
  // Create the new array - user must be told to delete it at some point
       int** newArray = new int *[sizeOfOriginalArrayX];
...
int obliczNEH(int **tablicaKolejnosci, int iloscZadan, int iloscMaszyn)
{
    ...
    else
    {
        ...
        int * tmp=tablicaTymczasowa[iloscZadan-i];
...
int main()
{
  ...
  /*      int * tmp=tablicaZadan[0];
        wypiszElementy(tablicaZadan,iloscZadan,iloscMaszyn);
        cout<<endl;
tablicaZadan=insert(tablicaZadan,0,tablicaZadan[2],iloscZadan);

        wypiszElementy(tablicaZadan,iloscZadan,iloscMaszyn);
        cout<<endl;
tablicaZadan =insert(tablicaZadan,2,tmp,iloscZadan);
